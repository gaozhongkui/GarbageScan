...
int extract7zBytes(Byte *decryptData, int decryptDataLength, Byte *un7zData, const char* storagePath, char *tempName) {
    ...
    sprintf(srcFilePath, "/%s%s%ld", storagePath, tempName, rawTime);

    if (access(srcFilePath, F_OK) == 0) {
        remove(srcFilePath);
    }

    FILE *pSrcFile = fopen(srcFilePath, "wb");

    if (decryptData == NULL || pSrcFile == NULL) {
        PrintError("decryptData == NULL || pSrcFile == NULL");
        return -1;
    }

    fwrite(decryptData, sizeof(Byte), (size_t) decryptDataLength, pSrcFile);
    fclose(pSrcFile);
    ...
    if (InFile_Open(&archiveStream.file, srcFilePath)) {
        PrintError("can not open input file");
        return -1;
    }
    ...
    if (res == SZ_OK) {
        ...
        size_t outSizeProcessed = 0;
        size_t len;
        len = SzArEx_GetFileNameUtf16(&db, 0, NULL);
        if (len > tempSize) {
            SzFree(NULL, temp);
            tempSize = len;
            temp = (UInt16 *) SzAlloc(NULL, tempSize * sizeof(temp[0]));
        ...
        res = SzArEx_Extract(&db, &lookStream.vt, 0,
                             &blockIndex, &outBuffer, &outBufferSize,
                             &offset, &outSizeProcessed,
                             &allocImp, &allocTempImp);
        if (res != SZ_OK)
        ...
        memcpy(un7zData, outBuffer + offset, outSizeProcessed);
